...
const SupportTicket = () => {
 ...
 const truncateText = (text) => {
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (response.data.tickets) {
        ...
        const uniqueIssueTypes = [...new Set(response.data.tickets
          .map(ticket => ticket.issueType)
          .filter(type => type))];
        setIssueTypes(uniqueIssueTypes);
        setCustomers([...new Set(response.data.tickets
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        `${API_BASE_URL}/${selectedRepliesTicket._id}/reply`,
        newMessage
      );
  ...
  const updateTicketStatus = async (ticketId, newStatus) => {
    try {
      await axiosInstance.put(`${API_BASE_URL}/${ticketId}/status`, { status: newStatus });
  ...
  const filteredTickets = tickets.filter((ticket) => {
    const matchesSearch = (
      ...
      (ticket.issueType?.toString().toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
    ...
    const matchesIssueType = selectedIssueType === "all" || ticket.issueType === selectedIssueType;
      ...
      <Card className="bg-white shadow-md">
        <CardHeader className="border-b border-gray-100">
          <div className="flex items-center">
            ...
            <CardTitle className="text-xl text-gray-800">Ticket Details</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Badge
                className={`${getStatusColor(selectedTicket.status)} px-4 py-1 rounded-full text-sm font-medium`}
              >
            ...
            <div className="grid gap-4 p-4 bg-gray-50 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{selectedTicket.userId?.name || 'Not Assigned'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 mb-1">Email</p>
                  <p className="font-medium">{selectedTicket.userId?.email || 'Not Available'}</p>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{selectedTicket.subject || 'No Subject'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500 mb-1">Description</p>
                <p className="font-medium whitespace-pre-wrap">{selectedTicket.description || 'No Description'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500 mb-1">Created At</p>
                <p className="font-medium">
                  {selectedTicket.createdAt ? new Date(selectedTicket.createdAt).toLocaleString() : 'Date not available'}
                </p>
              </div>
            </div>
          </div>
...
<div className="bg-white shadow-md rounded-lg">
      <div className="p-6 border-b border-gray-100">
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold text-gray-800">Ticket Support</h2>
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500">Total: {filteredTickets.length} tickets</span>
            </div>
          </div>
 ...
 <div className="flex gap-2 flex-wrap">
              {filterButtons.map((filter) => (
                <Button
                  key={filter.id}
                  variant={selectedIssueType === filter.id ? "default" : "outline"}
                ...
                >
                  ...
                  <Badge variant="secondary" className="ml-2 bg-gray-100">
                    {tickets.filter(ticket => 
                      filter.id === 'all' ? true : ticket.issueType === filter.id
                    ).length}
                  </Badge>
                </Button>
              ))}
          ...
          <tbody className="divide-y divide-gray-200">
            {currentTickets.length === 0 ? (
              ...
              currentTickets.map((ticket) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap cursor-pointer" onClick={() => handleTicketClick(ticket)}>
                    <span className="font-medium text-gray-900">{ticket.ticketId}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap cursor-pointer" onClick={() => handleTicketClick(ticket)}>
                    <span className="text-gray-800">{ticket.userId?.name || 'Not Assigned'}</span>
                  </td>
                  <td className="px-6 py-4 cursor-pointer" onClick={() => handleTicketClick(ticket)}>
                    <span className="text-gray-800">{ticket.issueType || 'N/A'}</span>
                  </td>
                  <td className="px-6 py-4 cursor-pointer" onClick={() => handleTicketClick(ticket)}>
                  ...
                  <td className="px-6 py-4 cursor-pointer" onClick={() => handleTicketClick(ticket)}>
                    <span className="text-gray-800 truncate block max-w-md" title={ticket.description}>
        ...
        <DialogContent>
          {selectedRepliesTicket && (
            <>
              <DialogHeader>
                <DialogTitle>
                  {selectedRepliesTicket.issueType}
                </DialogTitle>
                <DialogDescription>{selectedRepliesTicket.subject}</DialogDescription>
                <DialogDescription>
                    ...
                    {selectedRepliesTicket.messages.map((reply, index) => (
                      <li
                        ...
                        className={`p-3 rounded-md ${
                          reply.sender === "Support Agent" ? "bg-blue-100" : "bg-gray-100"
                        }`}
                      >
                        <p className="font-semibold">
                          {reply.sender === "Support Agent" ? "👑 Support Agent" : "Customer"}
                        </p>
